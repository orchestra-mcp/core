...
const Dashboard: FC = () => {
  ...
  const loadStats = useCallback(async () => {
    try {
      const [tasksRes, agentsOnlineRes, sessionsRes, memoriesRes] = await Promise.allSettled([
      ...
      setStats({
        active_tasks: tasksRes.status === 'fulfilled' ? (tasksRes.value.count ?? 0) : 0,
        agents_online:
          agentsOnlineRes.status === 'fulfilled' ? (agentsOnlineRes.value.count ?? 0) : 0,
        sessions: sessionsRes.status === 'fulfilled' ? (sessionsRes.value.count ?? 0) : 0,
        memories: memoriesRes.status === 'fulfilled' ? (memoriesRes.value.count ?? 0) : 0,
      })
      setConnected(true)
    } catch {
      setConnected(false)
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {loading && agents.length === 0
            ...
            : (agents.length > 0 ? agents.slice(0, 4) : []).map((agent, idx) => {
                const statusInfo = STATUS_STYLES[agent.status] || STATUS_STYLES.offline
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{
                background: connected ? 'var(--status-online)' : 'var(--destructive-default)',
                boxShadow: connected ? '0 0 6px hsla(153.1, 60.2%, 52.7%, 0.3)' : 'none',
              }}
            />
            <div>
