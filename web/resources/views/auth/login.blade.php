<x-layouts.guest title="Sign In">
    <h2 class="text-lg font-medium text-[--color-text-primary] text-center mb-6">Sign in to your account</h2>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-4">
        ...
        <div>
            <div class="flex items-center justify-between mb-1.5">
                <label for="password" class="block text-sm font-normal text-[--color-text-secondary]">Password</label>
                <a href="{{ route('password.request') }}" wire:navigate class="text-xs text-[--color-text-secondary] hover:text-[--color-text-primary] transition-colors">Forgot password?</a>
            </div>
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="remember"
                   class="rounded border-[--color-border] bg-[--color-bg-input] text-brand-purple focus:ring-brand-purple/40 focus:ring-offset-0 w-3.5 h-3.5">
            <span class="text-sm text-[--color-text-secondary]">Remember me</span>
        </div>
    ...
    <p class="mt-6 text-center text-sm text-[--color-text-faint]">
        ...
        <a href="{{ route('register') }}" wire:navigate class="text-[--color-text-primary] hover:text-brand-purple font-medium transition-colors">Sign Up</a>
    </p>
</x-layouts.guest>
